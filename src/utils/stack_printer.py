...
class Stack_printer:
  ...
  def print_stack(array_stack: list[int], stack_name: str = 'UNNAMED STACK', stack_width: int = 20) -> None:
    ...
    stack_width = Stack_printer.adjust_stack_width(stack_name, stack_width)
    Stack_printer.print_stack_top_and_title(stack_name, stack_width)
    stack_length: int = len(array_stack)

    if stack_length == 1:

      Stack_printer.print_only_stack_cell(array_stack[0], stack_width)
    elif stack_length > 1: 

      Stack_printer.print_stack_top_cell(array_stack[-1], stack_width)
      for i in range(stack_length - 2, 0, -1):

        cell_value: int = array_stack[i]
        Stack_printer.print_stack_middle_cell(cell_value, stack_width)
      Stack_printer.print_stack_bottom_cell(array_stack[0], stack_width)
    Stack_printer.print_stack_bottom(stack_width)
  ...
  def print_only_stack_cell(single_value: int, stack_width: int) -> None:
    ...
    Stack_printer.print_stack_cell_center(single_value, stack_width)
  ...
  def print_stack_top_cell(top_value: int, stack_width: int) -> None:
    ...
    stack_cell_underline:str = '│├' + '─'*stack_width + '┤│'
    print(stack_cell_top_line)
    Stack_printer.print_stack_cell_center(top_value, stack_width)
    print(stack_cell_underline)


  def print_stack_middle_cell(cell_value:int, stack_width: int) -> None:

    Stack_printer.print_stack_cell_center(cell_value, stack_width)
    stack_cell_underline:str = '│├' + '─'*stack_width + '┤│'
    print(stack_cell_underline)
